...
public sealed class EventsWatcher : BackgroundService
{
    ...
    {
        var intervalSec = _config.GetValue<int?>("WatcherIntervals:BNQSeconds") ?? 300;
        while (!stoppingToken.IsCancellationRequested)
        {
            try { await TickAsync(stoppingToken); }
            catch (Exception ex) { _logger.LogError(ex, "EventsWatcher tick failed"); }
