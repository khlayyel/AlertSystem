...
public sealed class TreasuryWatcher : BackgroundService
{
    ...
    {
        var intervalSec = _config.GetValue<int?>("WatcherIntervals:TRESSeconds") ?? 600;
        while (!stoppingToken.IsCancellationRequested)
        {
            try { await TickAsync(stoppingToken); }
            catch (Exception ex) { _logger.LogError(ex, "TreasuryWatcher tick failed"); }
