...
public sealed class HrWatcher : BackgroundService
{
    ...
    {
        var intervalSec = _config.GetValue<int?>("WatcherIntervals:GRHSeconds") ?? 300;
        while (!stoppingToken.IsCancellationRequested)
        {
            try { await TickAsync(stoppingToken); }
            catch (Exception ex) { _logger.LogError(ex, "HrWatcher tick failed"); }
    ...
    private async Task TickAsync(CancellationToken ct)
    {
        ...
        // Contracts ending soon (within N days)
