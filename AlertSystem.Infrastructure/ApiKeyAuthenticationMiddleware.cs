    ...
    public sealed class ApiKeyAuthenticationMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, IApiKeyValidator validator)
        {
            if (context.Request.Path.StartsWithSegments("/api"))
            {
                ...
                if (string.IsNullOrWhiteSpace(apiKey) || !await validator.ValidateAsync(apiKey))
                {
                    context.Response.StatusCode = StatusCodes.Status401Unauthorized;
                    context.Response.ContentType = "application/json";
                    await context.Response.WriteAsync(System.Text.Json.JsonSerializer.Serialize(new { error = "invalid_api_key" }));
                    return;
                }
            }
            await _next(context);
