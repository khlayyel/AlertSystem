...
namespace AlertSystem.Services
{
    public sealed class WebPushService : IWebPushService
    {
        ...
        public async Task<bool> SendNotificationAsync(string deviceToken, string title, string message, string? iconUrl = null, object? data = null)
        {
            try
            {
                ...
                var payload = JsonSerializer.Serialize(new
                {
                    title = title,
                    body = message,
                    icon = string.IsNullOrWhiteSpace(iconUrl) ? "/icon-192x192.png" : iconUrl,
                    ...
                    data = data ?? new { url = "/Dashboard", timestamp = DateTimeOffset.UtcNow.ToUnixTimeSeconds() }
                });
        ...
        public async Task<string> SubscribeUserAsync(int userId, string endpoint, string p256dh, string auth)
        {
            try
            {
                ...
                // Note: Hotel users don't have device tokens in our schema
                // The subscription is stored separately in WebPushSubscriptions
